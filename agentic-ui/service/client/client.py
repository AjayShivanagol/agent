...
class ConversationClient:
    ...
    async def _send_request(self, request: JSONRPCRequest) -> dict[str, Any]:
        async with httpx.AsyncClient() as client:
            try:
                # Propagate user header if present in current request context
                hdr = ''
                try:
                    hdr = current_userinfo.get()
                except Exception:
                    hdr = ''
                headers = {'x-userinfo': hdr} if hdr else None
                try:
                    # Debug: indicate if header is attached (do not print sensitive value)
                    print(f"[client] POST /{request.method} x-userinfo attached: {bool(hdr)}")
                except Exception:
                    pass
    ...
    async def delete_agent(
        self, payload: 'DeleteAgentRequest'
    ) -> 'DeleteAgentResponse':
